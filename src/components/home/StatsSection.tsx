...
function StatCard({ 
  ...
  return (
    <div 
      className={`text-center group transition-all duration-700 ease-out p-4 sm:p-5 rounded-2xl bg-white border border-primary/10 shadow-sm hover:shadow-md hover:border-primary/20 ${
        isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
      }`}
      style={{ transitionDelay: `${index * 100}ms` }}
    >
      <div className="w-10 h-10 sm:w-11 sm:h-11 mx-auto mb-2.5 rounded-lg bg-primary/8 flex items-center justify-center transform transition-all duration-500 group-hover:scale-105 group-hover:bg-primary/12">
        <Icon className="w-5 h-5 sm:w-5.5 sm:h-5.5 text-primary" />
      </div>
      <div className="text-2xl sm:text-3xl font-bold text-foreground mb-0.5 tracking-tight">
        {stat.isDecimal ? count.toFixed(1) : count}{stat.suffix}
      </div>
      <div className="text-muted-foreground/70 font-medium text-xs sm:text-sm">
        {stat.label}
      </div>
    </div>
  );
...
export function StatsSection() {
  ...
  return (
    ...
    >
      {/* Soft light green background with subtle gradient */}
      <div className="absolute inset-0 bg-gradient-to-b from-white via-[hsl(150,30%,97%)] to-[hsl(150,25%,95%)]" />
